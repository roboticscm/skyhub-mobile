...
const double _kMenuItemHeight = 28.0;
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  final List<SDropdownMenuItem<T>> items;
...
class SDropdownMenuItem<T> extends StatelessWidget {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const SDropdownMenuItem({
    Key key,
    this.value,
    @required this.child,
  }) : assert(child != null),
...
class SDropdownButton<T> extends StatefulWidget {
  ...
  }) : assert(items == null || items.isEmpty || value == null || items.where((SDropdownMenuItem<T> item) => item.value == value).length == 1),
...
class _DropdownButtonState<T> extends State<SDropdownButton<T>> with WidgetsBindingObserver {
  int _selectedIndex;
  ...
  @override
  void didUpdateWidget(SDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items.where((SDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.hint != null || (!_enabled && widget.disabledHint != null)) {
      ...
      _enabled ? widget.hint : SDropdownMenuItem<Widget>(child: widget.disabledHint ?? widget.hint);
...
class DropdownButtonFormField<T> extends FormField<T> {
  ...
  DropdownButtonFormField({
    Key key,
    T value,
    @required List<SDropdownMenuItem<T>> items,
    this.onChanged,
